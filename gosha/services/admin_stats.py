"""Admin/analytics use cases: platform stats, usage buckets, scrape health."""
...
async def record_pageview(user_id: int | None, path: str) -> None:
    """Analytics must never break the site — best-effort by design."""
    try:
        from gosha.events import get_event_store
        await get_event_store().emit(
            "web.pageview", actor_id=user_id, payload={"path": path[:200]}
        )
    except Exception:
        pass
...
async def usage_by_day(days: int) -> list[dict]:
    ...
    for event_type, actor_id, timestamp, payload_raw in rows:
        ...
        if event_type == "web.pageview":
            ...
            try:
                if json.loads(payload_raw or "{}").get("new_user"):
                    bucket["signups"] += 1
            except ValueError:
                pass
